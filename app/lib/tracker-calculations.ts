...
export function calculateAccountStats(settings: Settings, trades: Trade[]): AccountStats {
  ...
  const debtPaid = Math.max(0, originalDebt - settings.currentDebt);
  const debtProgressPercent = originalDebt > 0
    ? Math.min(100, Math.round((debtPaid / originalDebt) * 100))
    : 100;

  const winningTrades = currentCycleTrades.filter((trade) => trade.profitLoss > 0);
  const losingTrades = currentCycleTrades.filter((trade) => trade.profitLoss < 0);
  const winRate = currentCycleTrades.length > 0 ? (winningTrades.length / currentCycleTrades.length) * 100 : 0;

  let currentWinStreak = 0;
  let bestWinStreak = 0;
  let runningStreak = 0;

  for (const trade of [...currentCycleTrades].reverse()) {
  ...
  const averageLossPerLosingTrade = losingTrades.length > 0
    ...
    : 0;
  const profitFactor = computeProfitFactor(currentCycleTrades);
...
export function getFinancialSummary(data: TrackerData) {
  const remainingDebt = data.settings.currentDebt;
  const originalDebt = data.settings.originalDebt || data.settings.currentDebt;
  const totalPayoutDebtReduction = data.payouts.reduce((sum, payout) => sum + Math.max(0, payout.debtBefore - payout.debtAfter), 0);
  const totalDebtPaid = Math.max(0, originalDebt - remainingDebt);
  const debtCompletedPercent = originalDebt > 0
    ? Math.min(100, Math.round((totalDebtPaid / originalDebt) * 100))
    : 100;
  const tradingProfitUsd = data.stats.currentProfit;
  const tradingProfitInr = tradingProfitUsd * data.settings.usdToInr;
  const totalSalaryContribution = data.settings.salaryContributions;
  const totalAmountPaidTowardsDebt = totalDebtPaid;
  const averageMonthlyTradingIncome = data.trades.length > 0 ? tradingProfitUsd / Math.max(1, Math.round(data.trades.length / 3)) : 0;
  const averageMonthlyTotalContribution = averageMonthlyTradingIncome;
  const monthsRemaining = remainingDebt > 0 && averageMonthlyTotalContribution > 0
    ? Math.ceil(remainingDebt / (averageMonthlyTotalContribution * data.settings.usdToInr))
    : 0;

  return {
    totalDebtPaid,
    totalPayoutDebtReduction,
    remainingDebt,
...
export function getGoalsSummary(data: TrackerData) {
  const progressPercent = Math.min(100, Math.max(0, Math.round((data.stats.currentProfit / data.settings.minimumProfitForPayout) * 100)));
...
export function getCurrentCycleTrades(settings: Settings, trades: Trade[]): Trade[] {
  const startKey = settings.cycleStartDate || "";
  return trades.filter((trade) => {
    if (trade.cycleNumber != null) return trade.cycleNumber === settings.currentCycleNumber;
    return !startKey || tradeDateKey(trade) >= startKey;
  });
}
